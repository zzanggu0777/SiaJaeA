...
#include <TF\Settings_260315_0350T.mqh>
...
private:
   ...
   int               m_kb, m_ks, m_kc, m_ku, m_kd, m_kcl, m_kcs, m_kw, m_kl, m_kh; 
   bool              m_sdown, m_exp, m_drag, m_eq_drag;
   int               m_bx, m_by, m_dmx, m_dmy, m_dox, m_doy, m_eq_x, m_eq_y, m_eq_dox, m_eq_doy;
   double            m_vol_step, m_vol_min, m_vol_max; 
   int               m_vol_dig;

public:
   // Why: 생성자에서 기본 좌표 및 상태값을 초기화하여 객체 생성 시 즉시 사용 가능하게 함
   CStickerPanel() : m_pre("SP_"), m_sdown(false), m_exp(true), m_drag(false), m_bx(130), m_by(0), m_eq_drag(false), m_eq_x(-1), m_eq_y(-1) {}
   ~CStickerPanel() {}

   // Why: Main에서 전달한 설정값 주소를 연결하여 실시간 파라미터 공유
   void SetSettings(CStickerSettings *s) { m_set = s; }
   
   // Why: UIScale 배율을 적용하여 4K 모니터 등 고해상도 환경에서 UI 크기 자동 조절
   int S(int v) { return (int)(v * m_set.UIScale); } 
   ...
   bool Init(string suffix="")
   {
      ...
      string k;
      k = m_set.BuyKey;      if(k != "") { StringToUpper(k); m_kb = StringGetCharacter(k, 0); } else m_kb = 0;
      k = m_set.SellKey;     if(k != "") { StringToUpper(k); m_ks = StringGetCharacter(k, 0); } else m_ks = 0;
      k = m_set.CloseKey;    if(k != "") { StringToUpper(k); m_kc = StringGetCharacter(k, 0); } else m_kc = 0;
      
      k = (m_set.LotUpKey == "")   ? "U" : m_set.LotUpKey;   StringToUpper(k); m_ku = StringGetCharacter(k, 0);
      k = (m_set.LotDownKey == "") ? "D" : m_set.LotDownKey; StringToUpper(k); m_kd = StringGetCharacter(k, 0);
      
      k = m_set.CloseLongKey;  if(k != "") { StringToUpper(k); m_kcl = StringGetCharacter(k, 0); } else m_kcl = 0;
      k = m_set.CloseShortKey; if(k != "") { StringToUpper(k); m_kcs = StringGetCharacter(k, 0); } else m_kcs = 0;
      k = m_set.CloseWinKey;   if(k != "") { StringToUpper(k); m_kw = StringGetCharacter(k, 0); } else m_kw = 0;  
      k = m_set.CloseLossKey;  if(k != "") { StringToUpper(k); m_kl = StringGetCharacter(k, 0); } else m_kl = 0;  
      k = m_set.CloseHalfKey;  if(k != "") { StringToUpper(k); m_kh = StringGetCharacter(k, 0); } else m_kh = 0;
   ...
   { 
      // Why: EA가 아예 삭제될 때만 전체 청소, 설정만 바뀔 땐 버튼만 지워 깜빡임 방지
      if(r == REASON_REMOVE) {
         ObjectsDeleteAll(0, m_pre); 
      } else {
         string btns[] = {"CL", "CS", "W", "L", "E", "C", "S", "B", "TOG", "EQ_TXT"}; 
         for(int i = 0; i < 10; i++) ObjectDelete(0, m_pre + btns[i]);
      }
   ...
   void CloseHalfAll()
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--) {
         ...
         // Why: 정확히 물량의 절반을 Step 단위에 맞춰 계산하여 부분 청산 실행
         double half_vol = MathRound((vol * 0.5) / step) * step;
         if(half_vol >= min_vol) m_trade.PositionClosePartial(t, half_vol);
      }
      ChartSetInteger(0, CHART_BRING_TO_TOP, true);
   }
   ...
   void UpdateEquity()
   {
       ...
       if(!m_set.ShowEquityText) { if(ObjectFind(0, name) >= 0) ObjectDelete(0, name); return; }
       ...
       if(pnl > 0) { txt = StringFormat("[SP:%3d] EQ: $%.2f  [ +$%.2f (+%.2f%%) ]", spread, eq, pnl, pct); c = clrLime; }
       else if(pnl < 0) { txt = StringFormat("[SP:%3d] EQ: $%.2f  [ -$%.2f (%.2f%%) ]", spread, eq, MathAbs(pnl), pct); c = clrTomato; }
       else { txt = StringFormat("[SP:%3d] EQ: $%.2f", spread, eq); }

       ObjectSetString(0, name, OBJPROP_TEXT, txt);
       ObjectSetInteger(0, name, OBJPROP_COLOR, c); 
       ChartRedraw();
       
       last_eq = eq; last_pnl = pnl; last_spread = spread;
   }
   ...
   void OnChartEvent(const int id, const long &lp, const double &dp, const string &sp)
   {
      ...
      if(id == CHARTEVENT_KEYUP && m_set.UseScrollLock && lp == SM_VK_SCROLL) { m_sdown = false; return; }
      ...
      if(id == CHARTEVENT_MOUSE_MOVE)
      {
         if(sp == "1")
         {
            if(m_drag)
            {
               m_bx = m_dox - ((int)lp - m_dmx); m_by = m_doy + ((int)dp - m_dmy);
               if(m_bx < 0) m_bx = 0; if(m_by < 0) m_by = 0;
               Draw();
            }
            else if(m_eq_drag)
            {
               m_eq_x = m_eq_dox - ((int)lp - m_dmx); m_eq_y = m_eq_doy + ((int)dp - m_dmy);
               if(m_eq_x < 0) m_eq_x = 0; if(m_eq_y < 0) m_eq_y = 0;
            ...
            else
            {
               ...
               if(ObjectFind(0, togName) >= 0)
               {
                  ...
                  int left = (int)chartW - ox - ow; int right = (int)chartW - ox;
                  if((int)lp >= left && (int)lp <= right && (int)dp >= oy && (int)dp <= oy + oh)
                  {
                     m_drag = true; m_dmx = (int)lp; m_dmy = (int)dp; m_dox = m_bx; m_doy = m_by;
               ...
               if(m_set.ShowEquityText && ObjectFind(0, eqName) >= 0)
               {
                  ...
                  if(ow <= 0) ow = 120; if(oh <= 0) oh = 20;
                  int left = (int)chartW - ox - ow; int right = (int)chartW - ox;
                  int top = oy; int bottom = oy + oh;
                  if((int)lp >= left && (int)lp <= right && (int)dp >= top && (int)dp <= bottom)
                  {
                     m_eq_drag = true; m_dmx = (int)lp; m_dmy = (int)dp; m_eq_dox = m_eq_x; m_eq_doy = m_eq_y;
                     ChartSetInteger(0, CHART_MOUSE_SCROLL, false);
                  }
               }
            }
         ...
         { 
            if(m_drag) { m_drag = false; ChartSetInteger(0, CHART_MOUSE_SCROLL, true); }
            if(m_eq_drag) { m_eq_drag = false; ChartSetInteger(0, CHART_MOUSE_SCROLL, true); }
         }
      }
      else if(id == CHARTEVENT_OBJECT_CLICK && !m_drag && !m_eq_drag)
      {
         if(StringFind(sp, m_pre) == 0) { ObjectSetInteger(0, sp, OBJPROP_STATE, false); ChartRedraw(); }
         if(sp == m_pre + "TOG") { m_exp = !m_exp; Draw(); ChartSetInteger(0, CHART_BRING_TO_TOP, true); }
      ...
      else if(id == CHARTEVENT_OBJECT_ENDEDIT && sp == m_pre + "E") { ChartSetInteger(0, CHART_BRING_TO_TOP, true); }
   }

private:
   // Why: 0.0을 스캘핑 모드로 활용하기 위해 value <= 0 체크를 배제함
   ...
   void Entry(ENUM_ORDER_TYPE t)
   {
      ...
      double hard_sl = 0.0, hard_tp = 0.0;
      
      // Why: 끄는 것은 오직 'is_x' 스위치로만 판단하여 0.0 설정값을 보호함
   ...
   void Draw()
   {
      int h = 22; int w = 30; int Y = m_by; int cx = m_bx; color bg = m_set.ColorBtnBg; 
      if(m_exp) {
         ...
         for(int i = 0; i < 8; i++) HideBtn(btns[i]);
   ...
   { 
      string o = m_pre + n; 
      if(ObjectFind(0, o) < 0) { 
         ObjectCreate(0, o, OBJ_BUTTON, 0, 0, 0); ObjectSetInteger(0, o, OBJPROP_CORNER, CORNER_RIGHT_UPPER); 
         ObjectSetInteger(0, o, OBJPROP_FONTSIZE, S(8)); ObjectSetString(0, o, OBJPROP_FONT, "Arial Bold"); 
         ObjectSetInteger(0, o, OBJPROP_BORDER_COLOR, clrNONE); ObjectSetInteger(0, o, OBJPROP_SELECTABLE, false); 
         ObjectSetInteger(0, o, OBJPROP_HIDDEN, false); ObjectSetInteger(0, o, OBJPROP_COLOR, m_set.ColorBtnText); 
         ObjectSetInteger(0, o, OBJPROP_XSIZE, w); ObjectSetInteger(0, o, OBJPROP_YSIZE, h); ObjectSetInteger(0, o, OBJPROP_BGCOLOR, c); 
      } 
   ...
   { 
      string o = m_pre + n; 
      if(ObjectFind(0, o) < 0) { 
         ObjectCreate(0, o, OBJ_EDIT, 0, 0, 0); ObjectSetInteger(0, o, OBJPROP_CORNER, CORNER_RIGHT_UPPER); 
         ObjectSetInteger(0, o, OBJPROP_FONTSIZE, S(9)); ObjectSetString(0, o, OBJPROP_FONT, "Arial Bold"); 
         ObjectSetInteger(0, o, OBJPROP_ALIGN, ALIGN_CENTER); ObjectSetInteger(0, o, OBJPROP_BGCOLOR, clrWhiteSmoke); 
         ObjectSetInteger(0, o, OBJPROP_SELECTABLE, false); ObjectSetInteger(0, o, OBJPROP_HIDDEN, false); 
         ObjectSetInteger(0, o, OBJPROP_COLOR, clrBlack); ObjectSetString(0, o, OBJPROP_TEXT, DoubleToString(m_set.StartLots, 2)); 
   ...
   { 
      string o = m_pre + n; 
      if(ObjectFind(0, o) >= 0) { ObjectSetInteger(0, o, OBJPROP_XDISTANCE, -1000); ObjectSetInteger(0, o, OBJPROP_YDISTANCE, -1000); } 
   }
};
